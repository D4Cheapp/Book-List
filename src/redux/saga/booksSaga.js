...
//Запрос книги по id
function* fetchBookByIdSaga(action) {
    yield put(changeLoadingState(true));
...
function* booksSaga(){
    yield all([takeEvery(actionTypes.fetchBooks, fetchBooksSaga), takeEvery(actionTypes.addBook, addBookSaga),
        takeEvery(actionTypes.deleteBook, deleteBookSaga), takeEvery(actionTypes.editBook, editBookSaga),
            takeEvery(actionTypes.fetchBookById, fetchBookByIdSaga)]);
}
